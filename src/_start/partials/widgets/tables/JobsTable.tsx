...
const JobsTable: React.FC<Props> = ({ className }) => {
      ...
      .then((data) => {
        if (tableRef.current) {
          new DataTables(tableRef.current, {
            data: data,
            columns: [
              {title: 'Title Code', "data": "job_code"},
              {title: 'Title Name', "data": "job_desc"},
              {title: 'Duration', "data": "duration"},
              {
                title: 'Duration Avg.', "data": "duration_avg", render: function (data, type, row, meta) {
                  return parseFloat(data).toFixed(2).toLocaleString();
      
                }
              },
              {title: 'Qty. of Tasks', "data": "task_qty"},
              {title: 'Handle Time', "data": "handle_time"},
              {title: 'No. of Workflows', "data": "workflow_distinct_qty"},
              {title: 'No. of Tasks', "data": "task_distinct_qty"},
              {title: 'No. of Agents', "data": "agent_qty"},
    ...
    <div className={`card ${className}`}>
      ...
      <div className="card-header border-0 py-5 mb-2">
        <h3 className="card-title align-items-start flex-column">
          <span className="card-label fw-bolder text-dark"> Jobs</span>
          <span className="text-muted mt-3 fw-bold fs-7">
            Top 50 titles that were worked on in this scope. To expand, click here.
          </span>
        </h3>
