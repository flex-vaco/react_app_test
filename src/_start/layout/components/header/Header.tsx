...
export function Header() {
  ...
  let pathList = asPathWithoutQuery.split("/");
  pathList.shift();

  const sentenceCase = function (str: any) {
        ...
        <div className="d-flex align-items-center">
          ...
          {!config.aside.content && (
            <>
              ...
              <ol className="breadcrumb text-muted fs-3 fw-bold">
                {
                  pathList.map((item, index)=>{
                    currentPath += "/"+item;
                    return index+1 === pathList.length ? 
                    <li className="breadcrumb-item px-1 text-muted">{sentenceCase(item)}</li>
                    :
                    <li className="breadcrumb-item pe-1">
                    <a href={currentPath} className="">
                      {sentenceCase(item)}
                    </a>
                  </li>
                  })
